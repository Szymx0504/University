...
public class Game extends JFrame {
    ...
    Game(long fps, int chickens, long timeLimit) {
        ...
        this.scoreLabel = new JLabel("Score: " + Integer.toString(this.score)); 
        this.timeLabel = new JLabel("Time: " + timeLimit + "s"); 
    ...
    private void update() {
        ...
        long elapsedSeconds = (currentTime - gameTime) / 1000;
        long totalTimeLimit = this.timeLimit;
        
        if (elapsedSeconds >= totalTimeLimit) {
            return; 
        }
        
        long remainingSeconds = totalTimeLimit - elapsedSeconds;
        
        if (timeLabel != null) {
            timeLabel.setText("Time: " + remainingSeconds + "s");
        }
        
        float percentRemaining = 1.0f - ((float) elapsedSeconds / (float) totalTimeLimit);
        if (sandglass != null) {
            sandglass.updateVisual(percentRemaining);
        }
    ...
    private void resetGame(int chickens, long timeLimit) {
        ...
        if (sandglass != null) {
            panel.remove(sandglass);
        }
        this.sandglass = new Sandglass();
        this.panel.add(sandglass);
        
        scoreLabel.setText("Score: " + Integer.toString(score));
        timeLabel.setText("Time: " + timeLimit + "s");
