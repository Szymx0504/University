...
public:
    ...
    void setEye(char c)
    {
        ...
        {
            size_t pos = 0;
            while ((pos = art[i].find(oldEye, pos)) != std::string::npos)
            {
                art[i][pos] = c;
                pos++;
            }
    ...
    void reflect()
    {
        for (int i = 0; i < art.size(); i++)
        {
            std::reverse(art[i].begin(), art[i].end());
            for (char &c : art[i])
            {
                switch (c)
                {
...
class EyesCloser : public AnimalChanger
{
public:
...
class MirrorReflecter : public AnimalChanger
{
public:
    void changeAnimal(Animal &animal) const override
    {
