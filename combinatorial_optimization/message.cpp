...
void run_beam_improved(const vector<vector<Edge>>& g, int A, int B, const vector<ll>& dh1, const vector<ll>& dh2,
    ...
    candidates.reserve(beam_width * 30);
    ...
    while(tm.should_continue(0.008) && !beam.empty()) {
        ...
        for(auto &st : beam){
            ...
            for(const auto &e : g[u]){
                ...
                if(np >= best_score * 1.15) continue;
                ...
                heur += expect_pr * avg_extra * 0.6;
                
                double random_tiebreak = (rng() % 1000) / 800000.0;
    ...
    for(auto &st : beam){
        ...
        while(cur != B && steps < 4000 && tm.should_continue(0.001)){
            ++steps;
            int bestv = -1;
            ...
            for(const auto &e: g[cur]){
                ...
                bool prime = is_prime_cached(edgeIndex);
                ll add = prime ? 3LL * e.w2 : (ll)e.w1;
                double heur = min((double)dh1[v], (double)dh2[v]) * 0.7;
...
bool advanced_local_improve(vector<vector<Edge>>& g, vector<int>& path, ll& score, const TimeManager& tm, mt19937 &rng){
    ...
    ll original_score = score;
    
    for(int iter = 0; iter < MAX_ITERS && tm.should_continue(0.008); ++iter){
        bool local_improved = false;
        
        if(L >= 4 && (iter % 3 == 0 || !improved)){
            for(int attempt = 0; attempt < 4 && !local_improved; ++attempt){
                int i = uniform_int_distribution<int>(0, L-3)(rng);
                int j = uniform_int_distribution<int>(i+2, min(L-1, i + min(30, L-1-i)))(rng);
                ...
                while(!pq.empty() && tm.should_continue(0.0005)){
                    auto cur = pq.top(); pq.pop();
                    if(cur.d != dist[cur.v]) continue;
        ...
        if(!local_improved && L >= 5){
            for(int attempt = 0; attempt < 8 && !local_improved; ++attempt){
                int a = uniform_int_distribution<int>(1, L-3)(rng);
                int b = uniform_int_distribution<int>(a+1, min(L-2, a+15))(rng);
        ...
        if(!local_improved && L >= 3 && (iter % 4 == 0)){
        ...
        if(!local_improved && L >= 6 && (iter % 5 == 0)){
            int i = uniform_int_distribution<int>(0, L-4)(rng);
            int j = uniform_int_distribution<int>(i+3, min(L-1, i+8))(rng);
            if(j - i >= 3){
                vector<int> segment(path.begin() + i, path.begin() + j + 1);
                for(int shuffle = 0; shuffle < 3 && !local_improved; ++shuffle){
                    // Fixed: use std::shuffle instead of shuffle
                    std::shuffle(segment.begin() + 1, segment.end() - 1, rng);
                    vector<int> test = path;
                    copy(segment.begin(), segment.end(), test.begin() + i);
                    ll sc2 = score_of_path(test, g);
                    if(sc2 < score * 0.998){
                        path = move(test);
                        score = sc2;
        ...
        if(!local_improved && iter > 150 && score < original_score * 0.95) break;
    }
...
void multi_start_local_search(vector<vector<Edge>>& g, int A, int B, vector<int>& best_path, ll& best_score, const TimeManager& tm, mt19937 &rng) {
    ...
    for(int i = 0; i < 8 && tm.should_continue(0.15); ++i){
        vector<int> path;
        if(i < 3) {
            double alpha = i * 0.3;
            ...
            path = dijkstra_path(N, g, A, B, [i](const Edge& e)->ll{ 
                return (ll)e.w1 + (i-3) * (ll)e.w2; 
            });
        } else {
            path = bfs_path(N, g, A, B);
        }
        ...
        TimeManager local_tm(min(tm.remaining() * 0.12, 1.5));
...
void iterative_deepening_search(vector<vector<Edge>>& g, int A, int B, vector<int>& best_path, ll& best_score, const TimeManager& tm, mt19937 &rng) {
    if(!tm.should_continue(0.2)) return;
    
    vector<int> current_best = best_path;
    ll current_score = best_score;
    
    for(int depth = 0; depth < 3 && tm.should_continue(0.05); depth++){
        TimeManager depth_tm(tm.remaining() * 0.3);
        if(advanced_local_improve(g, current_best, current_score, depth_tm, rng)){
            if(current_score < best_score){
                best_score = current_score;
        ...
        if(!tm.should_continue(0.05)) break;
        
        vector<int> perturbed = current_best;
        if(perturbed.size() > 10){
            int pos1 = uniform_int_distribution<int>(1, perturbed.size()-2)(rng);
            int pos2 = uniform_int_distribution<int>(1, perturbed.size()-2)(rng);
            if(pos1 != pos2){
                swap(perturbed[pos1], perturbed[pos2]);
...
int main(){
    ...
    sieve_primes(max(2, N + 300));
    auto pref_primes = compute_prime_prefix(N + 2000);
    ...
    if(tm.remaining() > 0.2){
        double beam_total = min(tm.remaining() * 0.6, 8.0);
        ...
        for(const auto& [use_mix, alpha, width] : beam_configs){
            if(!tm.should_continue(0.1)) break;
    ...
    if(!best_path.empty() && tm.should_continue(0.15)){
        iterative_deepening_search(g, A, B, best_path, best_score, tm, rng);
    }

    if(!best_path.empty() && tm.should_continue(0.1)){
        advanced_local_improve(g, best_path, best_score, tm, rng);
    }
