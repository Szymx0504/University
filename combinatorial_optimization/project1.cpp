...
void run_beam_improved(const vector<vector<Edge>>& g, int A, int B, const vector<ll>& dh1, const vector<ll>& dh2,
    ...
    candidates.reserve(beam_width * 20);

    while(tm.should_continue(0.01) && !beam.empty()) {
        candidates.clear();
        ...
        for(auto &st : beam){
            ...
            for(const auto &e : g[u]){
                ...
                heur += expect_pr * avg_extra * 0.7;
                
                double est = (double)np + heur + (rng() % 1000) / 500000.0;
    ...
    for(auto &st : beam){
        ...
        while(cur != B && steps < 3000 && tm.should_continue(0.001)){
            ++steps;
            int bestv = -1;
            ...
            for(const auto &e: g[cur]){
                ...
                bool prime = is_prime_cached(edgeIndex);
                ll add = prime ? 3LL * e.w2 : (ll)e.w1;
                double heur = min((double)dh1[v], (double)dh2[v]);
                double val = (double)add + heur * 0.8;
                if(val < bestvscore){ 
                    bestvscore = val; 
                    bestv = v; 
                }
...
bool local_improve_optimized(vector<vector<Edge>>& g, vector<int>& path, ll& score, const TimeManager& tm, mt19937 &rng){
    int L = (int)path.size();
    if(L <= 2) return false;
    ...
    for(int iter = 0; iter < MAX_ITERS && tm.should_continue(0.005); ++iter){
        bool local_improved = false;
        
        if(L >= 4){
            for(int attempt = 0; attempt < 3 && !local_improved; ++attempt){
                int i = uniform_int_distribution<int>(0, L-3)(rng);
                int j = uniform_int_distribution<int>(i+2, min(L-1, i + min(25, L-1-i)))(rng);
                ...
                while(!pq.empty() && tm.should_continue(0.001)){
                    auto cur = pq.top(); pq.pop();
                    if(cur.d != dist[cur.v]) continue;
        ...
        if(!local_improved && L >= 5){
            for(int attempt = 0; attempt < 5 && !local_improved; ++attempt){
                int a = uniform_int_distribution<int>(1, L-3)(rng);
                int b = uniform_int_distribution<int>(a+1, min(L-2, a+12))(rng);
        ...
        if(!local_improved && L >= 3){
            for(int attempt = 0; attempt < 2 && !local_improved; ++attempt){
        ...
        if(!local_improved && iter > 100) break;
    }
...
void multi_start_local_search(vector<vector<Edge>>& g, int A, int B, vector<int>& best_path, ll& best_score, const TimeManager& tm, mt19937 &rng) {
    ...
    for(int i = 0; i < 5 && tm.should_continue(0.1); ++i){
        vector<int> path;
        if(i == 0) {
        ...
        TimeManager local_tm(tm.remaining() * 0.1);
        if(local_improve_optimized(g, current_path, current_score, local_tm, rng)) {
...
void focused_local_search(vector<vector<Edge>>& g, vector<int>& best_path, ll& best_score, const TimeManager& tm, mt19937 &rng) {
    if(best_path.empty()) return;
    
    vector<int> current_path = best_path;
    ll current_score = best_score;
    
    // Try multiple rounds of local search with different intensities
    for(int round = 0; round < 3 && tm.should_continue(0.05); round++) {
        TimeManager round_tm(tm.remaining() * 0.3);
        if(local_improve_optimized(g, current_path, current_score, round_tm, rng)) {
            if(current_score < best_score) {
                best_score = current_score;
                best_path = current_path;
            }
        ...
        if(current_path.size() > 10 && tm.should_continue(0.02)) {
            vector<int> perturbed = current_path;
            int idx1 = uniform_int_distribution<int>(1, perturbed.size()-2)(rng);
            int idx2 = uniform_int_distribution<int>(1, perturbed.size()-2)(rng);
            if(idx1 != idx2) {
                swap(perturbed[idx1], perturbed[idx2]);
                ll perturbed_score = score_of_path(perturbed, g);
                TimeManager perturb_tm(tm.remaining() * 0.15);
                if(local_improve_optimized(g, perturbed, perturbed_score, perturb_tm, rng)) {
                    if(perturbed_score < best_score) {
                        best_score = perturbed_score;
                        best_path = perturbed;
                        current_path = perturbed;
...
int main(){
    ...
    sieve_primes(max(2, N + 100));
    auto pref_primes = compute_prime_prefix(N + 1000);
    ...
    // Phase 1: Multi-start with quick local search
    if(tm.remaining() > 0.2){
        multi_start_local_search(g, A, B, best_path, best_score, tm, rng);
    }

    // Phase 2: Beam search with optimized parameters
    if(tm.remaining() > 0.15){
        double beam_total = min(tm.remaining() * 0.5, 5.0);
        ...
        for(const auto& [use_mix, alpha, width] : beam_configs){
            ...
            double allot = min(beam_total / beam_configs.size(), tm.remaining() * 0.85);
    ...
    // Phase 3: Focused local search with perturbation
    if(!best_path.empty() && tm.should_continue(0.1)){
        focused_local_search(g, best_path, best_score, tm, rng);
    }
